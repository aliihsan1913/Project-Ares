...
CRITICAL_TEMP  = 1.0
RECOVERY_TEMP  = 0.3
CAP_MAX        = 100.0   # süperkapasitör max şarj (%)
...
STATE_CFG = {
    #                       label               badge_col         aurora_col      shield_a
    State.IDLE:             ("IDLE",             (100,255,140),   (80, 200,120),   40),
    State.LOW_THREAT:       ("LOW THREAT",        (200,220, 60),  (180,200, 50),   80),
    State.STORM_ALERT:      ("STORM ALERT",       (255,160, 30),  (220,130, 20),  130),
    State.OVERDRIVE:        ("OVERDRIVE",         (180, 80,255),  (140, 60,255),  200),
    State.CRITICAL_FAILURE: ("CRITICAL FAILURE",  (255, 40, 40),  (255, 30, 20),    0),
    State.RECOVERY_MODE:    ("RECOVERY MODE",     ( 80,180,255),  ( 50,140,220),   60),
}
...
class PIDController:
    """
    Beam akımını (0..1) flux hatasına göre ayarlar.
    Kp, Ki, Kd — jüri sorarsa: deneysel tuning, edge-compute bütçesine uygun.
    """
    ...
    def update(self, setpoint, measured, dt):
        ...
        self.integral  = max(-2.0, min(2.0, self.integral))   # anti-windup
...
# ─── PARÇACIK ────────────────────────────────────────────────────────────────
class Particle:
    def __init__(self, flux_mult=1.0):
        spread     = HEIGHT * 0.72
        self.x     = float(SUN_X + 90)
        self.y     = MARS_Y + random.uniform(-spread/2, spread/2)
        spd        = random.uniform(3.8, 6.8) * (0.8 + flux_mult*0.4)
        ang        = random.uniform(-0.14, 0.14)
    ...
    def try_deflect(self, shield_r, state, beam_current):
        ...
        if dist < eff_r + 8 and not self.deflected:
            ...
            self.vx = nx * spd * random.uniform(0.6, 1.1)
            self.vy = ny * spd * random.uniform(0.6, 1.1) + random.uniform(-0.5,0.5)
            self.deflected = True
            self.life -= 0.15
...
def draw_sun(surf, state, sim_time):
    ...
    if state in (State.STORM_ALERT, State.OVERDRIVE):
        ...
        cr = int(60 + t*300)
        ca = int(120*(1-t))
        cme = pygame.Surface((cr*2, cr*2), pygame.SRCALPHA)
        pygame.draw.circle(cme, (255,160,20,ca), (cr,cr), cr, 2)
        surf.blit(cme, (SUN_X-cr, SUN_Y-cr+jitter))


# ─── ÇİZİM: MARS ─────────────────────────────────────────────────────────────
def draw_mars(surf, font_s, state):
    R  = MARS_R
    cx, cy = MARS_X, MARS_Y
    D  = R*2+4
    mc = (R+2, R+2)
...
# ─── ÇİZİM: AURORA & KALKAN ──────────────────────────────────────────────────
def draw_shield_aurora(surf, state, sim_time, shield_alpha, beam_current):
    if state == State.CRITICAL_FAILURE:
        return
    cfg       = STATE_CFG[state]
    aurora_col= cfg[2]
    ...
    count = {
        ...
        State.STORM_ALERT:22, State.OVERDRIVE:32, State.RECOVERY_MODE:10
    }.get(state, 10)
...
def draw_hud_left(surf, font, font_s,
    ...
    def row(label, value, color, bar_val=None, bar_max=1.0, bar_col=(70,160,255)):
        nonlocal yo
        surf.blit(font_s.render(label, True, HUD_LABEL), (px+10, py+yo)); yo += 14
        surf.blit(font.render(value,   True, color),      (px+10, py+yo)); yo += 18
    ...
    row("CYCLOTRON POWER",      f"{cyclotron_watts:.1f} W",(120,220,255),   cyclotron_watts, 5000.0,(100,200,255))
...
# ─── ÇİZİM: SAĞ PANEL (state badge + açıklama) ───────────────────────────────
...
def main():
    ...
    pygame.display.set_caption(
        "Adaptive Plasma Radiation Simulator — Phase 3: PID + Thermal")
    ...
    flt_flux       = 1.5
    temperature    = 0.0
    beam_current   = 0.0   # PID çıkışı [0..1]
    cyclotron_watts= 0.0
    rf_sync        = 1.0   # [0..1]
    cap_charge     = 0.0   # [0..100]
    shield_alpha   = 40.0
    state          = State.IDLE
    cme_timer      = random.uniform(15.0, 30.0)
    ...
    while True:
        ...
        while accumulator >= PHYS_DT:
            ...
            if manual_flux is not None:
                ...
                cme_timer -= PHYS_DT
                if cme_timer <= 0:
                    raw_flux  = random.uniform(2.5, 3.2)
                    cme_timer = random.uniform(20.0, 40.0)
                else:
                    raw_flux = max(0.1,
                                   1.5
            ...
            # ── PID KONTROLCÜ ────────────────────────────────────────────────
            # Setpoint: flux'u bastırmak için hedef akım
            # CRITICAL/RECOVERY'de PID sıfırlanır
            if state in (State.CRITICAL_FAILURE, State.RECOVERY_MODE):
                pid.reset()
                beam_current = 0.0
            else:
                # Setpoint: daha yüksek flux → daha yüksek beam hedefi
            ...
            # ── SİKLOTRON GÜCÜ ──────────────────────────────────────────────
            # W = beam_current * max_power + cap_bonus
            ...
            # ── SICAKLIK MODELİ (Schmitt Trigger) ────────────────────────────
        ...
        # Kalkan opaklığı smooth
        target_alpha = float(STATE_CFG[state][3])
        ...
        draw_shield_aurora(screen, state, sim_time, shield_alpha, beam_current)
        for p in particles: p.draw(screen)
        for f in impacts:   f.draw(screen)
        draw_mars(screen, font_s, state)
        ...
        hint = font_s.render(
            "1:IDLE  2:LOW THREAT  3:STORM ALERT  4:OVERDRIVE  0:AUTO  |  ESC:QUIT",
            True, (55,95,155))
